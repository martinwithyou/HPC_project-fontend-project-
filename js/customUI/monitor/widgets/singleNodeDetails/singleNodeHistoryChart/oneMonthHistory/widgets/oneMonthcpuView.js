...
var myChart = echarts.init(document.getElementById('oneHourHistory_cpu'));
// 指定图表的配置项和数据
...
var refreshTime=startTime[lastIndex_1];
	function refreshChart(){
             ...
             $.ajax({
				type:"get",
				url:urlConstants.oneMonth_cpu+"/"+Number($("#body").attr("data-node_id"))+"/tendency/month/cpu/"+"?starttime="+refreshTime,
				//url:currentUrl,
				success:function(res){
					...
					$("#oneHourHistory_cpu_result").html(lastValue);
...
function firstTimeGetDataSuccessfully(){
            ...
            $.ajax({
				type:"get",
				url:urlConstants.oneMonth_cpu+"/"+Number($("#body").attr("data-node_id"))+"/tendency/month/cpu/",
				async: false,
				success:function(res){
